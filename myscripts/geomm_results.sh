...
do
	echo "en-$lang"
	CUDA_VISIBLE_DEVICES=3 python geomm.py "$EMBEDDINGS/en.emb.txt" "$EMBEDDINGS/$lang.emb.txt" -dtrain "$DICTIONARIES/en-$lang.train.txt" -dtest "$DICTIONARIES/en-$lang.test.txt" --normalize unit center --max_opt_iter 150 --l2_reg 1e3 --max_vocab 0 --normalize_eval --eval_batch_size 1000 --geomm_embeddings_path output/en-$lang
done
